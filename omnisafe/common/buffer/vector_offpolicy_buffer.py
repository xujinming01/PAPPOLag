...
class VectorOffPolicyBuffer(OffPolicyBuffer):
    ...
    def __init__(  # pylint: disable=super-init-not-called,too-many-arguments
        ...
        if isinstance(act_space, Box):
            ...
            assert (
                len(act_space.spaces) == 2 and
                isinstance(act_space.spaces[0], Discrete) and
                act_space.spaces[0].shape == () and
                isinstance(act_space.spaces[1], Box)
            )
            act_buf = torch.zeros(
                (size, num_envs, 1 + act_space.spaces[1].shape[0]),
                dtype=torch.float32,
                device=device,
            )
        else:
