...
class Evaluator:  # pylint: disable=too-many-instance-attributes
    ...
    def __load_model_and_env(
        ...
        if hasattr(self._cfgs, 'algo') and self._cfgs['algo'] in [
            ...
            if 'AQ' in self._cfgs['algo']:
                critic_cfg = self._cfgs['model_cfgs']['critic']
                self._critic = CriticBuilder(
                    ...
                    weight_initialization_mode=weight_initialization_mode,
                    num_critics=critic_cfg['num_critics'],
                ).build_critic(critic_type='q')
                self._critic.load_state_dict(model_params['q'])
    ...
    def evaluate(
        ...
        Args:
            ...
            soc_init (float): The initial SOC. Defaults to 0.6.
        ...
        # import time
        # start = time.time()
        assert num_episodes == 1, 'Only support one episode evaluation now.'
        for episode in range(num_episodes):
            obs, _ = self._env.reset(
                seed=self._cfgs['seed'],
                options={'reset_zero_soc': soc_init, 'eval': True},
            )
            self._safety_obs = torch.ones(1)
            ep_ret, ep_cost, length = 0.0, 0.0, 0.0

            # obs = self._env.reset_zero(soc_init=soc_init)
            ...
            print(f'Episode reward: {ep_ret:.2f}')
            print(f'Episode cost: {ep_cost:.2f}')
            print(f'Episode length: {length}')
        # print(f'step time: {(time.time()-start)/length * 1000:.2f} ms')
