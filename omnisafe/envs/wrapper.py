...
class ActionScale(Wrapper):
    ...
    def __init__(
        ...
        super().__init__(env=env, device=device)
        assert isinstance(self.action_space, (spaces.Box, spaces.Tuple)), \
            'Only support Box or Tuple action space.'

        if isinstance(self.action_space, spaces.Box):
            box_act_space = self.action_space
        else:
            box_act_space = self.action_space[1]  # [0] is Discrete action space
        ...
        min_action = np.zeros(box_act_space.shape, dtype=box_act_space.dtype) + low
        max_action = np.zeros(box_act_space.shape, dtype=box_act_space.dtype) + high
        if isinstance(self.action_space, spaces.Box):
            ...
            self._action_space: spaces.Tuple = spaces.Tuple((
                self.action_space[0],
                spaces.Box(
                    low=min_action,
                    high=max_action,
                    shape=box_act_space.shape,
                    dtype=box_act_space.dtype,
                ),
            ))
    ...
    def step(
        ...
        """
        if isinstance(self.action_space, spaces.Box):
            scaled_act = self._old_min_action + (self._old_max_action - self._old_min_action) * (
                action - self._min_action
            ) / (self._max_action - self._min_action)
        else:
            act_disc = action[:1]
            act_cont = action[1:]
            act_cont = self._old_min_action + (self._old_max_action - self._old_min_action) * (
                act_cont - self._min_action
            ) / (self._max_action - self._min_action)
            scaled_act = torch.cat((act_disc, act_cont), dim=0)
