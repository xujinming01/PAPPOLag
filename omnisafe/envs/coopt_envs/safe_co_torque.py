...
class EMSEnv(gym.Env):
    ...
    def step(self, action: list):
        ...
        # if ed > 0 and Tm_act < 0.5:  # ed > 0 and decelerating
        #     Tm_act = 0.5
        # elif ed < 0 and

        if w < we_idle or Tm_act < 0.5:  # w<we_idle or decelerating
        # if w < we_idle:
            # Te = Te_idle
            Te = 0
            # we = we_idle
        ...
        Tm = (Tm_act - 0.5) / 0.5 * Tm_max  # Tm directly
        ...
        Tw = (Tm + Te_drive) * ig * ifd * (eta_g * eta_f) ** np.sign(Tm + Te_drive)
        ...
        if vh < 1e-2 and Tw < self._gettorque(vh, 0):  # vehicle stopped
            ...
            Tw = Tw_min
            # assert Te_drive == 0, f'Te_drive > 0: {Te_drive}'
            ...
            Tm = (Tw * eta_g * eta_f) / (ig * ifd) - Te_drive
            assert Tm >= -Tm_max, f'Tm < -Tm_max: {Tm}'
        Tb = 0
        ...
        #               - ke * (Pb / (3600 * 1000) * dt))  # electricity cost
        c_ems = p_soc + p_w
        r_ems = float(- mf + r_soc)
        # r_ems = float(- mf / 0.01 + r_soc - c_ems)
        ...
        dist_opt_inf = 5.2 + 0.72 * vh_n + 0.068 * vh_n ** 2
        dist_opt_sup = 6.8 + 0.88 * vh_n + 0.072 * vh_n ** 2
        ed_opt_to_inf = dist_opt - dist_opt_inf
        ed_opt_to_min = dist_opt - dist_min
        ...
        # # distance error, no error in optimal range
        # dist_deviation = dist_n - dist_opt
        # if dist_deviation < dist_opt_inf:
        #     ed = dist_opt_inf - dist_n
        # elif dist_deviation > dist_opt_sup:
        #     ed = dist_n - dist_opt_sup
        # else:
        #     ed = 0
        ...
        if abs(ed) > ed_opt_to_min:  # ed out of bound
            p_ed = cost_max * 2
            # ah_n = (vp_n - vp) / dt
            vh_n = vp_n
            dist_n = dist_opt
        ...
        # if abs(ed) <= ed_opt_to_inf:
        #     r_e = - abs(ed / ed_opt_to_inf)
        # else:
        #     r_e = (ed / ed_opt_to_inf) ** 2

        # penalty consistent in min-max range
        # r_e = - abs(ed / ed_opt_to_min)
        r_e = 0
        ...
        unscaled_state = [vh_n, ah_n, dist_n, soc, gear_idx]
    ...
    def reset(
        ...
        *,
        ...
        self.state = np.array([vh, ah, dist, soc, gear_idx], dtype=np.float64)
        self.count = 0
        self.eval = False
