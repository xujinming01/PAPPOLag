...
MIN_DWELL_STEPS = 10  # min steps for gear shift dwell time
# MIN_DWELL_STEPS = 0
...
class EMSEnv(gym.Env):
    def __init__(self, cycle):
        ...
        self.action_space = gym.spaces.Tuple((
            ...
            gym.spaces.Box(
                low=np.array([0, -ah_MAX], dtype=np.float32),
                high=np.array([1, ah_MAX], dtype=np.float32),
            ),  # engine torque, host acceleration
        ))
        ...
        self.steps_since_last_shift = MIN_DWELL_STEPS  # to allow immediate shift at start
        self.shift_count = 0
    ...
    def step(self, action: list):
        ...
        p_collision = 0
        p_ed = 0
        assert abs(ed) <= ed_opt2min, f'ed > ed_opt2min: {ed} > {ed_opt2min} at step {self.count}'
        ...
        #     # p_gear = 0
        #     gear_idx = next_gear
        if self.steps_since_last_shift >= MIN_DWELL_STEPS and next_gear != gear_idx:
            assert abs(next_gear - gear_idx) <= 1, "shift > 1"
            gear_idx = next_gear
            self.steps_since_last_shift = 0
            self.shift_count += 1
        else:
            self.steps_since_last_shift += 1
        shift = next_gear - gear_idx
        ...
        #     p_w = 0
        p_w = 0
        # assert w <= w_MAX + eps, f'w > w_MAX: {w:.5f} > {w_MAX} at step {self.count}'
        if w > w_MAX:
            p_w = cost_max
