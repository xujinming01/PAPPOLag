...
class EMSEnv(gym.Env):
    def __init__(self, cycle):
        ...
        # self.state = np.array([0, 0, 0])  # vh, ah, d, len=3
    ...
    def step(self, action: list):
        ...
        ah = self.state[1]
        soc = self._ems_state[2]
        gear_idx_prev = int(self._ems_state[3])
        # Tw = self._ems_state[4]  # torque demand at wheels

        # ----------------- action -----------------
        assert len(action) == 1, 'action length error'
        ...
        self._ems_state[:] = [vh, ah_n, soc, gear_idx_prev]
        ems_action = self._get_ems_action(self._ems_state)
        shift = int(np.argmax(ems_action[:3]))
        Te_act = action[-1]
        ...
        # ) / (l_e + l_jerk + l_mf + l_soc + l_Ah)  # normalized rewards
        # ) / (l_e + l_jerk)
        ...
        # new state
        # unscaled_state = [vh_n, ah_n, self.dist]
        unscaled_state = [vh_n, ah_n, self.dist, soc, gear_idx]
        self.state[:] = unscaled_state
        self._ems_state[:] = [vh_n, ah_n, soc, gear_idx]
    ...
    def reset(
        ...
        *,
        ...
        self.max_jerk = 0
        self.vp = 0
        # self.state = np.array([vh, ah, self.dist], dtype=np.float64)
        self.state = np.array([vh, ah, self.dist, soc, gear_idx], dtype=np.float64)
        self._ems_state = np.array([vh, ah, soc, gear_idx], dtype=np.float64)
        self.count = 0
        self.eval = False
    ...
    def _reset_zero(self, soc_init):
        ...
        self._ems_state = np.array([0, 0, soc_init, 0], dtype=np.float64)
        self.count = 0
        self.ii = 0
    ...
    @staticmethod
    def _load_model():
        """load the trained EMS model.
        :return: Evaluator object
        """

        evaluator = Evaluator()
        evaluator.load_saved(
            save_dir="runs/PPOLag-{SeqEMSCHTCLT-v1}/seed-004-2025-12-28-15-54-22",
            model_name='epoch-best.pt'
        )

        return evaluator

    def _get_ems_action(self, ems_state):
        """Get EMS action from the trained model.
        :param ems_state: current EMS state
        :return: EMS action
        """

        ems_state = torch.as_tensor(ems_state, dtype=torch.float32)
        with torch.no_grad():
            ems_action = self._ems_model._actor.predict(ems_state, deterministic=True)
