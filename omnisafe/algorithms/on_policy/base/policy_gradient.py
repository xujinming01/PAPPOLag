...
class PolicyGradient(BaseAlgo):
    ...
    def learn(self) -> tuple[float, float, float]:
        ...
        for epoch in range(self._cfgs.train_cfgs.epochs):
            ...
            # ep_return = self._logger._current_row['Metrics/EpRet']
            # ep_cost = self._logger._current_row['Metrics/EpCost']
            ep_return = list(self._logger._data['Metrics/TestEpRet'])[-1]
            ep_cost = list(self._logger._data['Metrics/TestEpCost'])[-1]
            if ep_cost <= best_cost and ep_return >= best_return:
                best_return = ep_return
                best_cost = ep_cost
                best_epoch = epoch
                self._logger.torch_save(epoch='best')
