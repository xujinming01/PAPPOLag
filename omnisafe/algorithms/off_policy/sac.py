...
class SAC(DDPG):
    ...
    def _init(self) -> None:
        ...
        if self._cfgs.algo_cfgs.auto_alpha:
            act_space_shape = self._env.action_space.shape
            if act_space_shape is None:
                act_space_shape = self._env.action_space.spaces[1].shape
            self._target_entropy = -torch.prod(torch.Tensor(act_space_shape)).item()
