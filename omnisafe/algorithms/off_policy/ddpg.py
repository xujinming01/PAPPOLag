...
class DDPG(BaseAlgo):
    ...
    def _init_log(self) -> None:
        ...
        if 'AQ' in self._cfgs.algo:
            what_to_save['q'] = self._actor_critic.reward_critic
        ...
        # self._logger.register_key('Metrics/p_w', window_length=50)
        # self._logger.register_key('Metrics/p_Tm', window_length=50)
        # self._logger.register_key('Metrics/p_soc', window_length=50)
        ...
        if self._cfgs.train_cfgs.eval_episodes > 0:
            ...
            # self._logger.register_key('Metrics/Testp_w', window_length=50)
            # self._logger.register_key('Metrics/Testp_Tm', window_length=50)
            # self._logger.register_key('Metrics/Testp_soc', window_length=50)
    ...
    def learn(self) -> tuple[float, float, float]:
        ...
        for epoch in tqdm(range(self._epochs)):
            ...
            #     self._logger.torch_save()
            ep_r_eval = list(self._logger._data['Metrics/TestEpRet'])[-1]
            ep_c_eval = list(self._logger._data['Metrics/TestEpCost'])[-1]
            if ep_c_eval <= best_c and ep_r_eval >= best_r:
                best_r = ep_r_eval
                best_c = ep_c_eval
                self._logger.torch_save(epoch=f'best')
