...
class Actor(nn.Module, ABC):
    ...
    def __init__(
        ...
        elif isinstance(self._act_space, spaces.Tuple) and len(self._act_space.spaces) == 2:
            self._act_dim: dict[str, int] = {}
            for space in self._act_space.spaces:
                if isinstance(space, spaces.Box):
                    self._act_dim['box'] = space.shape[0]
                elif isinstance(space, spaces.Discrete):
                    self._act_dim['discrete'] = int(space.n)
                else:
                    raise NotImplementedError(f'act_space: {space} is not supported')
        else:
            raise NotImplementedError
...
class Critic(nn.Module, ABC):
    ...
    def __init__(
        ...
        elif isinstance(self._act_space, spaces.Tuple):
            self._act_dim: dict[str, int] = {}
            for space in self._act_space.spaces:
                if isinstance(space, spaces.Box):
                    self._act_dim['box'] = space.shape[0]
