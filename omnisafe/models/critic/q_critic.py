...
class QCritic(Critic):
    ...
    .. note::
        ...
        The Actor-Q critic network contains multiple critics, and the output of the network :meth`forward`
        is a 2D tensor of Q-values. The first dimension is the number of critics, and the second dimension
        is a list of Q-values, each of which corresponds to a specific discrete action.
    ...
    def __init__(
        ...
        if isinstance(self._act_dim, int):  # common Q critic
            out_actor_dim = self._act_dim
            out_critic_dim = 1
        elif isinstance(self._act_dim, dict):  # actor-Q critic
            out_actor_dim = self._act_dim['box']
            if separate_cost:
                out_critic_dim = 1  # separate actor-Q and cost critic
            else:
                out_critic_dim = self._act_dim['discrete']
        else:
            raise NotImplementedError(f'Action space {self._act_dim} is not implemented.')
        for idx in range(self._num_critics):
            if self._use_obs_encoder:
                ...
                net = build_mlp_network(
                    [hidden_sizes[0] + out_actor_dim] + hidden_sizes[1:] + [out_critic_dim],
                    activation=activation,
                    weight_initialization_mode=weight_initialization_mode,
                )
                ...
                net = build_mlp_network(
                    [self._obs_dim + out_actor_dim, *hidden_sizes, out_critic_dim],
                    activation=activation,
                    weight_initialization_mode=weight_initialization_mode,
                )
    ...
    def forward(
        ...
        for critic in self.net_lst:
            if self._use_obs_encoder:
                ...
                # res.append(torch.squeeze(critic(torch.hstack([obs, act])), -1))
        return res

    def traced_predict(self, obs_act: torch.Tensor) -> list[torch.Tensor]:
        """Predict the Q-value given [obs, act] pair.

        Args:
            obs_act (torch.Tensor): [obs, act] pair.

        Returns:
            Q1-value of the observation-action pair.
        """
        return self.net_lst[0](obs_act)
