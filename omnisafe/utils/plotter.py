...
class Plotter:
    ...
    def plot_data(
        ...
        **kwargs: Any,
        ...
        # # 定义你想要放大的区间
        # ax1_x1, ax1_x2 = 600, 750
        # ax2_x1, ax2_x2 = ax1_x1, ax1_x2
        # ax1_y1, ax1_y2 = -400, -50
        # ax2_y1, ax2_y2 = -10, 60
        #
        # # --- 在第一张图上添加放大子图 ---
        # # 创建一个内嵌的 Axes 对象。参数 [x, y, width, height] 是相对于父 Axes 的坐标。
        # # [0.45, 0.15, 0.5, 0.4] 表示小图的左下角在 (45%, 15%) 的位置，宽高为主图的 50% 和 40%。
        # axins1 = sub_figures[0].inset_axes([0.5, 0.15, 0.35, 0.4])
        #
        # # 在内嵌 Axes 上绘制同样的数据
        # sns.lineplot(
        #     data=data_to_plot,
        ...
        #     errorbar='sd',
        #     ax=axins1,
        #     legend=False,  # 内嵌图不需要自己的图例
        #     # linewidth=1.5,
        #     err_kws={'alpha': 0.05},
        # )
        #
        # # 设置内嵌图的 x 轴范围
        # axins1.set_xlim(ax1_x1, ax1_x2)
        # axins1.set_ylim(ax1_y1, ax1_y2)
        # # 隐藏内嵌图的 x, y 轴标签
        # axins1.set_xlabel('')
        # axins1.set_ylabel('')
        # # 可选：减小刻度标签的字体大小
        ...
        #
        # # # 在主图上绘制指示框，表明放大区域
        # _, connectors1 = sub_figures[0].indicate_inset_zoom(axins1, edgecolor="black", alpha=0.5)
        # connectors1[0].set_visible(False)  # lower left
        # connectors1[1].set_visible(True)  # upper left
        # connectors1[2].set_visible(True)  # lower right
        # connectors1[3].set_visible(False)  # upper right
        #
        # # --- 在第二张图上添加放大子图 ---
        # # 创建第二个内嵌 Axes 对象
        # axins2 = sub_figures[1].inset_axes([0.5, 0.5, 0.35, 0.4])
        #
        # # 在内嵌 Axes 上绘制数据
        # sns.lineplot(
        #     data=data_to_plot,
        ...
        #
        # # 设置范围和格式
        # axins2.set_xlim(ax2_x1, ax2_x2)
        # axins2.set_ylim(ax2_y1, ax2_y2)
        # axins2.set_xlabel('')
        # axins2.set_ylabel('')
        # axins2.tick_params(axis='both', which='major', labelsize=10)
        #
        # # 绘制指示框
        # _, connectors2 = sub_figures[1].indicate_inset_zoom(axins2, edgecolor="black", alpha=0.5)
        # connectors2[0].set_visible(True)
        # connectors2[1].set_visible(False)
        ...
        # sub_figures[0].set_yscale('symlog')
        # sub_figures[0].set_ylim([-1.3, -0.3])
        if xaxis == 'Train/Epoch':
            sub_figures[0].set_ylabel('Returns')
            sub_figures[0].set_xlabel('Episodes')
            sub_figures[1].set_xlabel('Episodes')
            sub_figures[0].set_xlim([0, 800])
            sub_figures[1].set_xlim([0, 800])
            # sub_figures[0].set_ylim([-800, 0])
            # sub_figures[1].set_ylim([0, 400])
    ...
    def make_plots(
        ...
        if type(cost_limit) is float:
            axes[1].axhline(y=cost_limit, ls='-.', c='black', linewidth=1, label='Cost limit')
        # axes[0].axhline(y=-7844, ls='--', c='black', linewidth=2, label='DP (Optimal)')  # DP optimal
        # axes[0].legend()
        # axes[1].legend()
        ...
        #     pad_inches=0.0,
        # )
        fig.savefig(os.path.join(save_dir, f'{save_name}.png'))
